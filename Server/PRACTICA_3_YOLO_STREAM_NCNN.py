"""
PRACTICA_3_YOLO_STREAM_NCNN.py — Comportamiento deliberativo con YOLO (NCNN) + streaming.

Igual que PRACTICA_3_YOLO_NCNN.py pero además envía el vídeo con los
bounding boxes dibujados al cliente de Freenove para verlo en tiempo real.

Usa NCNN como motor de inferencia (más rápido en ARM).

Instrucciones:
  1. Ejecutar en la RPi: sudo python PRACTICA_3_YOLO_STREAM_NCNN.py
...
Uso:
  sudo python PRACTICA_3_YOLO_STREAM_NCNN.py           → busca indefinidamente
  sudo python PRACTICA_3_YOLO_STREAM_NCNN.py --bolas 5 → para tras recoger 5 bolas
"""
import cv2
import numpy as np
import time
...
def main():
    ...
    try:
        while True:
            ...
            # Si no hay bola, evadir con umbral normal.
            # ==========================================================
            ...
            #   - Cámara confirma QUÉ hay (bola, no obstáculo)
            #   - Sonar confirma A QUÉ DISTANCIA está
            #   - Ambos regulan velocidad y deciden recogida
            # ==========================================================
            if bola_encontrada:
                tiempo_sin_bola = time.time()
