...
def irAPelotaVisible(estado, PWM, servo):
    ...
    pelota = datos[0]
    cx = pelota.get("cx", 300)  # Centro horizontal de la pelota
    cy = pelota.get("cy", 300)  # Centro vertical de la pelota
...
def main():
    ...
    with Manager() as manager:
        ...
        while not pelotaCogida:
            ...
            if peligroPorSonar():
                evasion2(estado,PWM)
            elif peligroPorImagen(estado):
                evasion1(PWM)
            else:
                if not mensaje:
                    mensaje = recogerMensaje(cliente)

                else:
                    if pelotaVisible(estado):
                        pelotavista=True
                        pelotaCogida = irAPelotaVisible(estado,PWM,servo)
                    elif pelotavista:
                        retroceder(PWM)
                        pelotavista=False
