...
def main():
    ...
    try:
        while True:
            ...
            # Evaluamos choque con otro robot usando sonar (a menos de 20cm)
            distancia_sonar = sonar.get_distance()
            choque_inminente = (distancia_sonar >= 0) and (distancia_sonar < 20.0)

            #COMPORTAMIENTO REACTIVO
            if limite_detectado or choque_inminente:
                if choque_inminente:
                    print(f"\n¡Objeto a {distancia_sonar:.1f} cm! Evadiendo...")
                else:    
            ...
            # Imprimir constantemente los valores (usa \r para no saturar la pantalla)
            # Solo imprime si no está realizando una maniobra de giro para no reescribir el print de arriba
            if not (limite_detectado or choque_inminente):
                print(f"Píxeles verdes: {cantidad_pixeles} | Distancia frontal: {distancia_sonar:4.1f}cm      ", end="\r")
